...
vignette: >
  %\VignetteIndexEntry{Description of the Assessment Models in MSEtool}
...
A variant of the SCA is the SRA, which fixed the predicted catches to the observed catches and uses Pope's approximation to calculate the annual harvest rate in the midpoint of the year.
...
where $Z_{a,t} = F_{a,t} + M_a$, and $M$ is natural mortality. Equation 59 is Baranov's equation with the substitution: $N_{a,t}=N_{a+1,t+1}\exp(Z_{a,t})$. Newton's method is used to numerically solve Equation 59. 
...
From $N_{A,t+1}$, $C_{A,t}$, $C_{A-1,t}$, $M_A$, and $M_{A-1}$, we can solve $F_{A-1,t}$ and $F_{A,t}$ as the values that satisfy equations 61-64. The corresponding abundances $N_{A-1,t}$ and $N_{A,t}$ are obtained with equation 60.
