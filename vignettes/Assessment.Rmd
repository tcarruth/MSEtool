...
There has been a rich history of development for the delay difference model for catch and index data. For the formulation used in MSEtool, the most relevant citations are Chapter of Hilborn and Walters (1992) and Carruthers et al. (2012).

## Growth
Growth in weigh-at-age $w_a$ follows the recursive Ford-Brody equation:
...
$$ s_t = (1 - u_t)\exp(-M), $$
where $u_t$ is the annual harvest rate and $M$ is the instantaneous natural mortality rate. By conditioning the model on effort $f_t$ (where effort is the ratio 
of the catch and index), the harvest rate is
$$ u_t = 1 - \exp(-qf_t), $$
...
The model is parameterized such $u_{MSY}$ and $MSY$ are estimated productivity parameters from which the 
stock-recruit parameters for the model can be derived (Martell et al. 2008).
...
where $B_{t-k}(1-u_{t-k})$ is the population biomass after fishing in year $t-k$.

From $u_{MSY}$ and $MSY$, the stock-recruit parameters $\alpha$ and $\beta$ are:
$$ \begin{align}
...
with respect to $u$, respectively, evaluated at $u_{MSY}$, defined as:
...
To derive $\alpha$ and $\beta$, define the equilibrium yield $Y$ as
$$Y = u B = u \phi_u R$$
where $\phi_u$ and $R$ are the biomass-per-recruit and the equilibrium recruitment as functions of $u$:
$$ R = \dfrac{\alpha \phi_u(1-u) - 1}{\beta \phi_u(1-u)}$$
Equation 12 is obtained by setting $\frac{dY}{du} = 0$ and solving for $\alpha$ ($u$ by definition is $u_{MSY}$). Equation 13 is obtained by solving for $\beta$ in equation 16 in the case where $Y = MSY$, $\phi_u = \phi_{MSY}$, $R = R_{MSY}$, and $u = u_{MSY}$.
...
## Dynamics equations
The surplus production model uses theFletcher (1978) formulation. The biomass $B_t$ in year $t$ is 
...
## Dynamics equations
The statistical catch-at-age model used in MSEtool assumes pulse fishing at the beginning of each annual time-step, following the parameterization of of Forrest et al. (2008). The model uses a time series of total catch (in weight), index, and catch-at-age observations, as well as information on weight, maturity, natural mortality at age.
...
\end{cases},
$$
where $f(a; \mu, \sigma)$ is the normal probability density function scaled to one at $\mu$. The $\mu$ and $\sigma$ for both the ascending and descending limbs of the double-normal form are estimated parameters.
...
The estimated index $\hat{I}_t$ is
$$ \hat{I}_t = \hat{q} \widehat{VB}_t.$$
The log-likelihood $L$ of the observed index $I_t$ over years $t = 1, ..., T$, assuming a lognormal distribution, is
$$L(I_t)\propto -T \log\sigma - 0.5\sum_{t=1}^{T}\left(\dfrac{\log(I_t/\hat{I}_t)}{\sigma}\right)^2.$$

## Estimation of recruitment and reference points

There are two variants of the statistical catch-at-age model for estimation of recruitment and reference points. In function `SCA`, recruitment is estimated as annual deviations from mean recruitment, similar to Cadigan (2016). MSY reference points are estimated outside the model. In `SCA2`, the parameters of the stock-recruit function, MSY reference points, and recruitment deviations are all estimated internally in the model. `SCA` generally will have a faster runtime than `SCA2`.

### External reference points (SCA)

Recruitment $R_t$ in year $t$ is
$$R_t = \bar{R} \exp(\delta_t - 0.5 \tau^2), $$
where $\delta_t \sim N(0, \tau^2)$ are recruitment deviations in lognormal space from the estimated mean recruitment $\bar{R}$ and $\tau$ is the standard deviation of the deviations.

### Internal reference points (SCA2)

In `SCA2`, parameters $u_{MSY}$ and $MSY$ are estimated, from which stock-recruit parameters are obtained. The mathematical description of the models are described below, and the full derivation is described in Forrest et al. (2008).
...
$$ \begin{align}
\alpha &= \dfrac{1 - \left(\dfrac{\psi'_{MSY}}{\psi_{MSY}} - \dfrac{\phi'_{MSY}}{\phi_{MSY}}\right) u_{MSY}}{\phi_{MSY}\left(1 + \dfrac{\psi'_{MSY}}{\psi_{MSY}}u_{MSY}\right)}\\
\beta &= \dfrac{\alpha \phi_{MSY} - 1}{R_{MSY} \phi_{MSY}},
\end{align}$$
where $\phi_{MSY}$ is the spawning biomass per recruit at $u = u_{MSY}$, 

$$\phi_{MSY} = \sum_{a=1}^A m_a w_a l_a$$
and
$$
l_a = 
\begin{cases} 
      1 & a = 1\\
      l_{a-1} (1 - v_{a-1} u_{MSY}) \exp(-M_{a-1}) & a = 2, \ldots, A-1\\
      \dfrac{l_{a-1} (1 - v_{a-1} u_{MSY}) \exp(-M_{a-1})}{1 - (1 - v_a u_{MSY}) \exp(-M_a)} & a = A \\
\end{cases}; 
$$
$\phi'_{MSY}$ is the derivative of $\phi$ with respect to $u$ and evaluated at $u_{MSY}$, 
$$ \phi'_{MSY} = \sum_{a=1}^A m_a w_a l'_a $$
and
$$
l'_a = 
\begin{cases} 
      0 & a = 1\\
      l'_{a-1} (1 - v_{a-1} u_{MSY}) \exp(-M_{a-1}) - l_{a-1} v_{a-1} \exp(-M_{a-1}) & a = 2, \ldots, A-1\\
      \dfrac{[l'_{a-1} (1 - v_{a-1} u_{MSY}) \exp(-M_{a-1}) - l_{a-1} v_{a-1} \exp(-M_{a-1})](1 - v_a u_{MSY})(1 - \exp(-M_a)) - l_{a-1} (1 - v_{a-1} u_{MSY}) \exp(-M_{a-1}) v_a \exp(-M_a)}
{[1 - (1- v_a u_{MSY}) \exp(-M_a)]^2} & a = A \\
\end{cases}; 
$$
$\psi_{MSY}$ is the vulnerable biomass per recruit at $MSY$
$$\psi_{MSY} = \sum_{a=1}^A v_a w_a l_a;$$
and $\psi'_{MSY}$ is the derivative of vulnerable biomass per recruit evaluated $u_{MSY}$ 
$$\psi'_{MSY} = \sum_{a=1}^A v_a w_a l'_a.$$

Parameter $\alpha$ is derived from the yield equation
$$Y = u \psi_u R $$
where is $\psi_u$ defined by equation 54 for any given $u$ and recruitment $R$ is
$$ R = \dfrac{\alpha \phi_u - 1}{\beta \phi_u}, $$ 
by setting $\frac{dY}{du} = 0$ and solving for $\alpha$; all quantities correspond to those at $MSY$. Equation 49 is obtained by solving for $\beta$ in Equation 57 in the $MSY$ scenario where

$$R_{MSY} = \dfrac{VB_{MSY}}{\psi_{MSY}} = \dfrac{MSY}{u_{MSY} \psi_{MSY}}. $$
...
Using the derivation analogous to that in Forrest et al (2008), parameters $\alpha$ and $\beta$ are solved as functions of $u_{MSY}$ and $MSY$, where
$$ \begin{align}
\alpha &= \dfrac{1}{\phi_{MSY}}\exp\left(\dfrac{-u_{MSY} \psi_{MSY} \phi'_{MSY}}
{\phi_{MSY}[u_{MSY} \psi'_{MSY} + psi_{MSY}] - u_{MSY} \phi'_{MSY} \psi_{MSY}}\right)\\
\beta &= \dfrac{\log(\alpha \phi_{MSY})}{R_{MSY} \phi_{MSY}}.
\end{align}$$
