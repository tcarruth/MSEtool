---
title: "Appendix: Description of the Assessment Models"
subtitle: "Assess-class objects in MSEtool"
...
There has been a rich history of development for the delay difference model for catch and index data. For the formulation used in MSEtol, the most relevant citations are Chapter of Hilborn and Walters (1992) and Carruthers et al. ().
...
Assuming a Beverton-Holt stock recruit relationship, then recruitment (at age $k$) in year $t$ is:
...
There are two variants of the statistical catch-at-age model for estimation of recruitment and reference points. In function `SCA`, recruitment is estimated as annual deviations from mean recruitment, similar to Cadigan (2014). MSY reference points are estimated outside the model. In `SCA2`, the parameters of the stock-recruit function, MSY reference points, and recruitment deviations are all estimated internally in the model. `SCA` generally will have a faster runtime than `SCA2`.
...
\end{align}$$
with $\phi_{MSY} = \sum_{a=1}^A m_a w_a l_a$ is the spawning biomass per recruit at $MSY$, and
$$
l_a = 
\begin{cases} 
      1 & a = 1\\
...
\end{cases}; 
$$
$\phi'_{MSY} = \sum_{a=1}^A m_a w_a l'_a$ is the derivation of $\phi$ with respect to $u$ and evaluated at $u_{MSY}$, and
$$
l'_a = 
\begin{cases} 
      0 & a = 1\\
...
\end{cases}; 
$$
$\psi_{MSY} = \sum_{a=1}^A v_a w_a l_a$ is the vulnerable biomass per recruit at $MSY$; and $\psi'_{MSY} = \sum_{a=1}^A v_a w_a l'_a$ is the derivative of vulnerable biomass per recruit evaluated $u_{MSY}$.
