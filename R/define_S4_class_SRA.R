          ...
          function(x, compare = TRUE, filename = "SRA_scope", dir = tempdir(), sims = 1:x@OM@nsim, Year = NULL, open_file = TRUE, quiet = TRUE, ...) {
            ...
            if(compare) {
              ...
              compare_rmd <- c("## Updated OM {.tabset}\n",
                               ...
                               "```\n",
                               "",
                               "```{r, fig.cap = \"Difference in catch (relative to observed time series mean), calculated as $C^{OM}_y/ \\bar{C}^{obs}_y - 1$. Positive values indicate higher catch in the OM.\"}",
