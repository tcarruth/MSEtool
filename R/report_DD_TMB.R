...
generate_report_DD_TMB <- function(Assessment, figure = TRUE, save_figure = FALSE,
                                   save_dir = getwd()) {
  ...
  relU <- data.frame(Year = Year, Value = report$relU_DD, Description = "U/UMSY")
  ...
  derived <- data.frame(Value = c(report$h, report$Bo_DD, report$Ro_DD, report$No_DD,
  ...
  if(figure) {
    ...
    plot_SR(SSB, expectedR, report$Ro_DD, report$Bo_DD)
    if(save_figure) {
    ...
    if(save_figure) {
      create_png(filename = file.path(plot.dir, "assessment_7_recruitment.png"))
      plot_timeseries(R$Year, R$Value, label = "Recruitment")
      dev.off()
      assess.file.caption <- rbind(assess.file.caption,
                                 c("assessment_7_recruitment.png", "Time series of recruitment."))
    }
...
retrospective_DD_TMB <- function(Assessment, nyr, figure = TRUE,
  ...
  # ts includes: Calendar Year, B, N, R, U, relU, relB, dep
  retro_ts <- array(NA, dim = c(nyr+1, ny_DD + k_DD, 8))
  ...
  for(i in 0:nyr) {
    ...
    if(opt$convergence == 0) {
      ...
      retro_ts[i+1, , ] <- cbind(Year, B, relB, dep, R, N, U, relU)
...
plot_yield_DD <- function(data, report, umsy, msy, u.vector = seq(0, 1, 0.01),
  ...
  BPR <- (Surv * Alpha_DD/(1 - Surv) + wa_DD)/(1 - Rho_DD * Surv)
  ...
  if(xaxis == "Depletion") {
    plot(Biomass[ind]/report$Bo_DD, Yield[ind], typ = 'l',
         xlab = expression(B/B[0]), ylab = "Equilibrium yield")
    segments(x0 = BMSY/report$Bo_DD, y0 = 0, y1 = msy, lty = 2)
    segments(x0 = 0, y0 = msy, x1 = BMSY/report$Bo_DD, lty = 2)
    abline(h = 0, col = 'grey')
  }
  invisible()
}
