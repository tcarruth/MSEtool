#' (In development) Delay - Difference Stock Assessment with UMSY and MSY leading using TMB
#'
...
DD_TMB <- function(x, Data, reps = 100) {
  ...
  params <- list(model = "DD", log_UMSY_DD = log(Data@Mort[x]),
  ...
  Obj <- MakeADFun(data = info$data, parameters = info$params, DLL = "MSEtool",
                  silent = TRUE) #makes TMB function
  Opt <- nlminb(start = Obj$par, objective = Obj$fn, gradient = Obj$gr) #optimizes
  SD <- sdreport(Obj) #gets parameter estimates and std. error using ADREPORT
