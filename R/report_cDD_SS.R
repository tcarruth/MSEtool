...
summary_cDD_SS <- function(Assessment) {
  ...
  output <- list(model = "Continuous Delay-Differential (State-Space)",
...
generate_plots_cDD_SS <- function(Assessment, save_figure = FALSE, save_dir = tempdir()) {
  assign_Assessment_slots()

  if(save_figure) {
    prepare_to_save_figure()
    index.report <- summary(Assessment)
    html_report(plot.dir, model = "Continuous Delay Differential (State-Space)", current_status = index.report$current_status,
  ...
  if(save_figure) {
    html_report(plot.dir, model = "Continuous Delay Differential (State-Space)", captions = data.file.caption,
  ...
  if(save_figure) {
    html_report(plot.dir, model = "Continuous Delay Differential", captions = assess.file.caption,
...
retrospective_cDD_SS <- function(Assessment, nyr, figure = TRUE, save_figure = FALSE, save_dir = tempdir()) {
  assign_Assessment_slots()
  data <- info$data
  ny <- data$ny
  k <- data$k
  ...
  for(i in 0:nyr) {
    ...
    obj2 <- MakeADFun(data = data, parameters = info$params, map = obj$env$map, random = obj$env$random, DLL = "MSEtool", silent = TRUE)
...
plot_retro_cDD <- function(retro_ts, retro_est, save_figure = FALSE, save_dir = tempdir(), nyr_label, color, fix_h, SR) {
  ...
  if(save_figure) {
    ...
    html_report(plot.dir, model = "Continuous Delay Differential (State-Space)", captions = ret.file.caption,
