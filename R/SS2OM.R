...
SS2OM<-function(SSdir,nsim=48,proyears=50,length_timestep=NA,Name=NULL,Source="No source provided",
                 Author="No author provided",printstats=FALSE,verbose=TRUE){
  ...
  #}
  nseas <- max(replist$timeseries$Seas) #alternative way to get the number of seasons
  ...
  #L50 <- rep(GP$Mat1[1], 2)
  OM@a <- rep(GP$WtLen1[1], 2)
  OM@b <- rep(GP$WtLen2[1], 2)
...
#' @author T. Carruthers
#' @export getBH
getBH<-function(pars,SSB,rec,SSBpR,mode=1,plot=FALSE){
  ...
  if(mode==1){
    ...
    return(-sum(dnorm(log(rec)-log(recpred),0,sigmaR,log=T)))#-dnorm(pars[1],0,6,log=T)) # add a vague prior on h = 0.6
...
#' @author T. Carruthers
#' @export optBH
optBH<-function(x,SSB,rec,SSBpR,R0temp,pars,frac=0.5,plot=FALSE){
  ...
  opt<-optim(pars,getBH,method="L-BFGS-B",lower=c(-6.,log(R0temp/50)),upper=c(6.,log(R0temp*50)), hessian=T,
           SSB=SSB[samp],rec=rec[samp],SSBpR=SSBpR,mode=1, plot=FALSE)
...
#' @import snowfall
#' @export SRopt
SRopt<-function(nsim,SSB,rec,SSBpR,plot=FALSE,type="BH"){
