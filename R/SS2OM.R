...
SS2OM <- function(SSdir, nsim = 48, proyears = 50, reps = 1, maxF = 3, seed = 1, Obs = DLMtool::Generic_Obs, Imp = DLMtool::Perfect_Imp,
  ...
  if(nrow(GP)>1){
    ...
    print(GP)
    message("Row ", gender, " will be used (see gender argument in SS2OM).\n")
    GP <- GP[gender, ]
  }
  ...
  # Depletion --------------------------------------
  if(packageVersion("r4ss") == 1.24) { # Check if equilibrium F > 0 in year 1 of model
    InitF_ind <- grepl("InitF", replist$parameters$LABEL)
  } else InitF_ind <- grepl("InitF", replist$parameters$Label)
  ...
  if(season_as_years) {
    ...
    } else rec_ind <- match(rec_ind, replist$recruit$Yr)
    ...
    } else rec_ind <- match(year_first_rec_dev:replist$endyr, replist$recruit$Yr)
