...
mahplot<-function(outlist,MPs){

  nMP<-length(outlist)
  ncol<-floor(nMP^(1/3))
  nrow<-ceiling(nMP/ncol)
  par(mai=c(0.2,0.2,0.01,0.01),omi=c(0.2,0.2,0.2,0.01))
  layout(matrix(1:(nrow*ncol*2),nrow=nrow*2),heights=rep(c(2,1),nrow))
  
  for(mm in 1:nMP){
    
    mahdensplot(outlist[[mm]])
    PRBplot(outlist[[mm]])
...
mahdensplot<-function(out,adj=0.9,alpha=0.05){  
  ...
  for(pp in 1:np){
    ...
    if(ymaxN>xmaxN)xmaxN=ymaxN
    if(ymaxA>xmaxA)xmaxA=ymaxA
  }
  
  for(pp in 1:np){
    #densN[[pp]]$y<-densN[[pp]]$y/max(densN[[pp]]$y)*heightadj
    #densA[[pp]]$y<-densA[[pp]]$y/max(densA[[pp]]$y)*heightadj
    densN[[pp]]$y<-densN[[pp]]$y/xmaxN*heightadj
    densA[[pp]]$y<-densA[[pp]]$y/xmaxA*heightadj
  }
  
  plot(c(0.5,np+0.3),c(0,tfac),col='white')
...
PRBplot<-function(out){
  
  plot((1:5) - 0.2,1-out$PRB[1,],col="#0000ff50",ylim=c(0,1),type='l',lwd=2,xlab="",ylab="")
  abline(h=0.8,lty=2)
  lines((1:5) - 0.2,out$PRB[2,],col="#ff000050",lwd=2)
