...
#'
#' If you have convergence difficulties with \code{SP_SS}, try using a smaller value of \code{tau}.
#' @author Q. Huynh
#' @references
...
#' @useDynLib MSEtool
SP <- function(x = 1, Data, start = NULL, fix_Binit_frac = TRUE, fix_n = TRUE, silent = TRUE,
...
#' @importFrom stats nlminb
#' @useDynLib MSEtool
SP_SS <- function(x = 1, Data, start = NULL, fix_Binit_frac = TRUE, fix_n = TRUE, fix_sigma = FALSE,
                  fix_tau = TRUE, integrate = FALSE, silent = TRUE, control = list(eval.max = 1000),
                  inner.control = list(), ...) {
  ...
  if(is.character(opt) || is.character(SD)) {
    ...
    if(integrate) {
      Dev <- c(rep(0, sum(is.na(map$log_B_dev))), SD$par.random)
      SE_Dev <- c(rep(0, sum(is.na(map$log_B_dev))), sqrt(SD$diag.cov.random))
    } else {
      par <- SD$par.fixed[names(SD$par.fixed) == "log_B_dev"]
      SE_par <- sqrt(diag(SD$cov.fixed)[names(SD$par.fixed) =="log_B_dev"])

      Dev <- c(rep(0, sum(is.na(map$log_B_dev))), par)
      SE_Dev <- c(rep(0, sum(is.na(map$log_B_dev))), SE_par)
    }
    Assessment <- new("Assessment", Model = "SP_SS",
                      UMSY = report$UMSY, MSY = report$MSY, BMSY = report$BMSY, VBMSY = report$BMSY,
                      B0 = report$K, VB0 = report$K, U = structure(report$U, names = Year),
