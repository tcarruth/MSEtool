...
SS2Data <- function(SSdir, Name = "Object generated by SS2Data function", Common_Name = "", Species = "", Region = "",
  ...
  if(!all(is.na(CSD))) {
    Csd_weighted <- weighted.mean(CSD[!is.na(CSD)], colSums(cbind(cat_weight, cat_numbers)))
  ...
  if(is.numeric(index_fleet) && index_fleet %in% unique(replist$cpue$Fleet)) {
    ...
    mean(sqrt(exp(cpue_subset$SE^2) - 1))

    Data@CV_Ind <- mean(sqrt(exp(cpue_subset$SE^2) - 1), na.rm = TRUE)
    message(paste0("CV of Index (mean of CVs converted from lognormal SE), Data@CV_Ind = ", round(Data@CV_Ind, 2)))
  }
