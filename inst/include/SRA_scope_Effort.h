...
//{
  ...
  for(int ff=0;ff<nfleet;ff++) q_effort(ff) = exp(log_q_effort(ff));
  ...
  for(int ff=0;ff<nfleet;ff++) {
    ...
    if(E_eq(ff)>0) F_equilibrium(ff) = q_effort(ff) * E_eq(ff);
  }
  ...
  for(int ff=0;ff<nfleet;ff++) {
    ...
    for(int y=0;y<n_y;y++) F(y,ff) = q_effort(ff) * E_hist(y,ff);
  }
