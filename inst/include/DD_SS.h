...
//{
  ...
  for(int tt=0; tt<ny_DD; tt++){
    ...
    Rec_dev_DD(tt) = exp(log_rec_dev(tt) - 0.5 * pow(sigma_R, 2));

    R_DD[tt + k_DD] = Arec_DD * Sp_DD(tt)/(1 + Brec_DD * Sp_DD(tt)) * Rec_dev_DD(tt);
  ...
  for(int tt=0; tt<ny_DD; tt++) {
    ...
    jnll_comp(1) -= dnorm(log_rec_dev(tt), Type(0), sigma_R, true);
  }
  jnll_comp(2) -= dlognorm(UMSY_DD, lUMSYprior, UMSYprior(1), true);
